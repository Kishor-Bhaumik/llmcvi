...
def fit_faiss_gpu_kmeans(embeddings, k, previous_centroids, seed, device):
    """Full-corpus GPU Lloyd k-means; k-means++ first, warm starts thereafter.

    Preserve 10 first-epoch initializations and 1 warm-start initialization.
    FAISS assignments use GPU IndexFlatL2; centroid updates are managed by
    FAISS. Input remains raw projector embeddings, as in the original code.
    """
    if device.type != "cuda":
        raise RuntimeError("FAISS GPU k-means requires a CUDA device.")
    if not hasattr(faiss, "StandardGpuResources"):
        raise RuntimeError(
            "This FAISS installation lacks GPU support. Install a "
            "CUDA-compatible GPU-enabled FAISS build."
        )
    x = np.ascontiguousarray(embeddings, dtype=np.float32)
    n, d = x.shape
    if not 2 <= k <= n:
        raise ValueError("k must be between 2 and the number of texts.")
    ...
    trials = 10 if previous_centroids is None else 1
    best_objective = float("inf")
    best_centroids = None

    for trial in range(trials):
        if previous_centroids is None:
            initial, _ = kmeans_plusplus(x, n_clusters=k, random_state=rng)
        else:
            initial = previous_centroids
        initial = np.ascontiguousarray(initial, dtype=np.float32)

        clustering = faiss.Clustering(d, k)
        clustering.niter = 100
        clustering.nredo = 1
        clustering.seed = seed + trial
        # Prevent FAISS's default per-centroid subsampling: use every text.
        clustering.max_points_per_centroid = max(256, (n + k - 1) // k)
        faiss.copy_array_to_vector(initial.ravel(), clustering.centroids)

        index = faiss.index_cpu_to_gpu(resources, gpu_id, faiss.IndexFlatL2(d))
        clustering.train(x, index)
        centers = faiss.vector_to_array(clustering.centroids).reshape(k, d).copy()

        # Compare final full-corpus inertia across initializations.
        index.reset()
        index.add(np.ascontiguousarray(centers))
        distances, _ = index.search(x, 1)
        objective = float(distances.sum(dtype=np.float64))
        if objective < best_objective:
            best_objective = objective
            best_centroids = centers
        del index, clustering
...
def pretrain(backbone, projector, loader, args, device):
    ...
    for epoch in range(args.pretrain_epochs):
        ...
        current_centroids = fit_faiss_gpu_kmeans(
            proj_embs, args.k, current_centroids, args.seed, device
        )
